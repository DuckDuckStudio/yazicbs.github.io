...
let Guess_number_random_number: number;
...
// 创建一个函数,生成指定范围内的随机整数
function Guess_number_init(min: number, max: number) {
    guess_max = max;
    guess_min = min;
...
function Guess_number(n: number) { // eslint-disable-line @typescript-eslint/no-unused-vars -- 用户在控制台使用
    // 判断猜测结果
    if (Number.isInteger(n)) {
        if (n < Guess_number_random_number) {
            console.log("[Game\\Guess Number] 太小了, 再猜看看?");
        } else if (n > Guess_number_random_number) {
